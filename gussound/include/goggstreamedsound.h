...
namespace GOpenAl
{
  ...
  ///
  class GUSSOUND_EXPORT_SYMBOL GOggStreamedSound
  ...
  public:
    ...
    const gussound::SoundDuration getEstimatedDurationInSeconds() const
    {
  ...
  protected:
    ...
    /// Do not change this to a smart pointer, as the repository is likely
    /// to store the sounds as smart pointers and that would produce issues.
    ...
    gussound::SoundDuration estimatedDurationInSeconds_;
  ...
  private:
    ...
    // There is a buffer being played and one waiting to be played. When the buffer being played
    // has finished the next one starts.
    // While the next buffer is being played, the first one is refilled with a new chunk of data
    // from the stream and is set to play once the one playing is finished.
    ALuint buffers_[2];
