...
namespace Serpents
{
  ...
  void WorkbenchState::enter (const app::SerpStateParams& param)
  {
    ...
    if (hover_sound.length () > 0)
    {
      ...
      try
      {
        gussound::SoundManager::getPtr()->getRepository()->addSound("menu_hover", file_name, true, gussound::GSC_Effect);
      }
    ...
    if (hover_sound.length () > 0)
    {
      ...
      try
      {
        gussound::SoundManager::getPtr()->getRepository()->addSound("menu_click", file_name, true, gussound::GSC_Effect);
      }
      catch (std::exception& e)
      {
        GTRACE (1, "Caught exception: " << e.what ());
  ...
  void WorkbenchState::exit ()
  {
    ...
    GTRACE (4, "Removing " << compositorList.size () << " compositors from the " << numShadowRTTs << " shadow texture viewports");
  ...
  void WorkbenchState::reactToKeyPress (OIS::KeyCode keycode)
  {
    ...
    if (keycode == OIS::KC_SPACE)
    {
      ...
      {
        Ogre::Resource* res= Ogre::TextureManager::getSingleton().createOrRetrieve ("secondary_view_render_texture",Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME).first.getPointer();
  ...
  void WorkbenchState::addSceneCompositors ()
  {
    ...
    std::vector<std::string> secondarySceneCompositorList = levelConfig_["general"]["applyCompositorToSecondaryView"].getAsStringList ();
    GTRACE (4, "Applying " << secondarySceneCompositorList.size () << " compositors to the secondary scene viewports");
    {
      for (std::vector<std::string>::iterator it = secondarySceneCompositorList.begin (); it != secondarySceneCompositorList.end (); ++ it)
      {
  ...
  bool WorkbenchState::initialize ()
  {
    ...
    //applyShadowTechnique (shadowType);
    ...
    double shadowDirLightTextureOffset = levelConfig_["lighting"]["shadowDirLightTextureOffset"].getAsDoubleOrDefaultVal (0);
    ...
    try
    {
      ...
      optionsLayoutFile = levelConfig_[skinConfiguration]["optionslayout"].getAsStringOrDefaultVal ("workbenchoptions.layout");
    ...
    if (btn)
    {
      ...
      btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonQuitClicked, this));
    ...
    {
      btn->setWantsMultiClickEvents(false);
      btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonOptionsClicked, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
    else
    {
    ...
    {
      btn->setWantsMultiClickEvents(false);
      btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonSubmenuClicked, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
    else
    {
    ...
    {
      btn->setWantsMultiClickEvents(false);
      btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonReloadGroupClicked, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
    else
    {
    ...
    {
      btn->setWantsMultiClickEvents(false);
      btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonPrevShadowTech, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
    else
    {
    ...
    {
      btn->setWantsMultiClickEvents(false);
      btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonNextShadowTech, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
    else
    {
    ...
    if (cbo)
    {
      cbo->addItem (new utils::SimpleListItem ("Easy", defaultBrushSkin, "ComboboxSelectionBrush", CEGUI::Colour(0xFFFF0101), ""));
      cbo->addItem (new utils::SimpleListItem ("Normal", defaultBrushSkin, "ComboboxSelectionBrush", CEGUI::Colour(0xFFFF0101), ""));
      cbo->addItem (new utils::SimpleListItem ("Hard", defaultBrushSkin, "ComboboxSelectionBrush", CEGUI::Colour(0xFFFF0101), ""));

      for (int i = 1; i <= 10; ++ i)
      {
        std::string name = "SubItem";
        name.append (guslib::stringutil::IntToString (i));
        cbo->addItem (new utils::SimpleListItem (name, defaultBrushSkin, "ComboboxSelectionBrush", CEGUI::Colour(0xFFFF0101), ""));
      }
      int itemIndex = 1;
      // Set the list to read-only, so that the user can't type in entries.
      cbo->setItemSelectState (itemIndex, true);
    ...
    // Update the level label
    btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopRightSelectionFrame/OptionsWindowTab/Sample_Tab_1/ButtonGetRandomProgressValue");
    addWidgetMouseEffects(btnName1);
    btn = static_cast<CEGUI::PushButton*> (utils::CEGUIHelperUtil::getWindow (btnName1));
    if (btn)
    {
      btn->setWantsMultiClickEvents(false);
      btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonGetRandomValueClicked, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
    else
    {
    ...
    btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopRightSelectionFrame/OptionsWindowTab/Sample_Tab_1/Sample_ProgressSlider");
    addWidgetMouseEffects(btnName1);
    ...
    if (scroller)
    {
      ...
      scroller->subscribeEvent(CEGUI::Scrollbar::EventScrollPositionChanged, CEGUI::Event::Subscriber(&WorkbenchState::onScrollValueChange, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
    else
    {
    ...
    btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopRightSelectionFrame/OptionsWindowTab/Sample_Tab_2/SoundVolumeSlider");
    addWidgetMouseEffects(btnName1);
    scroller = static_cast<CEGUI::Scrollbar*> (utils::CEGUIHelperUtil::getWindow (btnName1));
    if (scroller)
    {
    ...
    btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopRightSelectionFrame/OptionsWindowTab/Sample_Tab_2/MusicVolumeSlider");
    addWidgetMouseEffects(btnName1);
    scroller = static_cast<CEGUI::Scrollbar*> (utils::CEGUIHelperUtil::getWindow (btnName1));
    if (scroller)
    {
    ...
    btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopLeftFrame/SFX_RB_Grouper_1/__auto_contentpane__/SFX_RB_Radio1");
    addWidgetMouseEffects(btnName1);
    ...
    btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopLeftFrame/SFX_RB_Grouper_1/__auto_contentpane__/SFX_RB_Radio2");
    addWidgetMouseEffects(btnName1);
    rbutton = static_cast<CEGUI::RadioButton*> (utils::CEGUIHelperUtil::getWindow (btnName1));
    if (rbutton)
    {
    ...
    btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopLeftFrame/SFX_RB_Grouper_1/__auto_contentpane__/SFX_RB_Radio3");
    addWidgetMouseEffects(btnName1);
    rbutton = static_cast<CEGUI::RadioButton*> (utils::CEGUIHelperUtil::getWindow (btnName1));
    if (rbutton)
    {
    ...
    if (listbox)
    {
      for (int i = 1; i <= 20; ++ i)
      {
        ...
        listbox->addItem (new utils::SimpleListItem (name, defaultBrushSkin, "ListboxSelectionBrush", CEGUI::Colour(0xFFFF0101), ""));
      }
    }
    else
    {
    ...
    if (itemListBox)
    {
      int numItemCount = itemListBox->getItemCount ();
      if (numItemCount > 0)
      {
        CEGUI::ItemEntry* itemAtZero = itemListBox->getItemFromIndex (0);
        if (itemAtZero)
        {
          //CEGUI::Image* selectionImage = const_cast<CEGUI::Image*> (itemAtZero->getse->brush());
        
          //if (selectionImage)
          //{
          //  GTRACE (3, "First item brush [" << selectionImage->getName().c_str () << "]");
          //}
        }
      }
      ...
      if (sb)
      {
        GTRACE (3, "Scrollbar props: doc size [" << sb->getDocumentSize () << "], page size [" << sb->getPageSize () << "]");
        GTRACE (3, "Scrollbar props: step size [" << sb->getStepSize () << "], overlap size [" << sb->getOverlapSize () << "]");
      }
      else
      {
    ...
    if (mclbox)
    {
      ...
      mclbox->setItem(new utils::SimpleListItem ("Laggers World", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 0, 0);
      mclbox->setItem(new utils::SimpleListItem ("yourgame.some-server.com", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 1, 0);
      mclbox->setItem(new utils::SimpleListItem ("[colour='FFFF0000']1000ms", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 2, 0);

      // Set second row item texts for the MCL
      mclbox->setItem(new utils::SimpleListItem ("Super-Server", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 0, 1);
      mclbox->setItem(new utils::SimpleListItem ("whizzy.fakenames.net", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 1, 1);
      mclbox->setItem(new utils::SimpleListItem ("[colour='FF00FF00']8ms", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 2, 1);

      // Set third row item texts for the MCL
      mclbox->setItem(new utils::SimpleListItem ("Cray-Z-Eds", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 0, 2);
      mclbox->setItem(new utils::SimpleListItem ("crayzeds.notarealserver.co.uk", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 1, 2);
      mclbox->setItem(new utils::SimpleListItem ("[colour='FF00FF00']43ms", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 2, 2);

      // Set fourth row item texts for the MCL
      mclbox->setItem(new utils::SimpleListItem ("Fake IPs", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 0, 3);
      mclbox->setItem(new utils::SimpleListItem ("123.320.42.242", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 1, 3);
      mclbox->setItem(new utils::SimpleListItem ("[colour='FFFFFF00']63ms", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 2, 3);

      // Set fifth row item texts for the MCL
      mclbox->setItem(new utils::SimpleListItem ("Yet Another Game Server", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 0, 4);
      mclbox->setItem(new utils::SimpleListItem ("abc.abcdefghijklmn.org", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 1, 4);
      mclbox->setItem(new utils::SimpleListItem ("[colour='FFFF6600']284ms", defaultBrushSkin, "MultiListSelectionBrush", CEGUI::Colour(0xFFFF0101), ""), 2, 4);
    }
    else
    {
      GTRACE (3, "could not find item named " << btnName1 << " for connection");
    ...
    if (listbox)
    {
      for (int i = 1; i <= 20; ++ i)
      {
        std::string name = "Item Added via code ";
        name.append (guslib::stringutil::IntToString (i));
        listbox->addItem (new utils::SimpleListItem (name, "GlossySerpent", "ListboxSelectionBrush", CEGUI::Colour(0xFFFF0101), ""));
      }
    ...
    if (item)
    {
      ...
      item->subscribeEvent(CEGUI::Window::EventMouseEntersArea, CEGUI::Event::Subscriber(&WorkbenchState::onWidgetHover, this));
      item->subscribeEvent(CEGUI::Window::EventMouseClick, CEGUI::Event::Subscriber(&WorkbenchState::onWidgetClick, this));
      GTRACE (3, "subscribed to click events for " << btnName1 << ".");
    }
  ...
  void WorkbenchState::setupSecondaryView (const std::string& hostWidgetName)
  {
    ...
    Ogre::TexturePtr char_texture = 
      Ogre::TextureManager::getSingleton().createManual ("secondary_view_render_texture"
    ...
    char_rt ->setAutoUpdated(true); // init = false


    Ogre::Viewport *char_view = char_rt->addViewport (enginePtr_->getSecondaryCamera ());
    //Ogre::Viewport *char_view = char_rt->addViewport (enginePtr_->getMainCamera ());
    ...
    //xxx-begin
    std::vector<std::string> compositorList = levelConfig_["lighting"]["applyCompositorToSecondaryView"].getAsStringList ();
    for (std::vector<std::string>::iterator it = compositorList.begin (); it != compositorList.end (); ++ it)
    {
    ...
    if (myWidget)
    {
      ...
      float myWidth = surfaceSize.d_width * myWidget->getSize ().d_width.d_scale + myWidget->getSize ().d_width.d_offset;
      float myHeight = surfaceSize.d_height * myWidget->getSize ().d_height.d_scale + myWidget->getSize ().d_height.d_offset;
  ...
  {
    std::vector<std::string> sceneCompositorList = levelConfig_["general"]["applyCompositorToScene"].getAsStringList ();
    GTRACE (4, "Applying compositor enabled status to [" << bCompositorStatus << "] for " << sceneCompositorList.size () << " compositors to the scene texture viewports");
    {
    ...
    std::vector<std::string> secondary_scene_compositor_list = levelConfig_["general"]["applyCompositorToSecondaryView"].getAsStringList ();
    GTRACE (4, "Applying compositor enabled status to [" << bCompositorStatus << "] for " << secondary_scene_compositor_list.size () << " compositors to the secondary scene texture viewports");
    {
      Ogre::Viewport *vp = enginePtr_->getSecondaryCamera ()->getViewport ();
      for (std::vector<std::string>::iterator it = secondary_scene_compositor_list.begin (); it != secondary_scene_compositor_list.end (); ++ it)
      {
        try
        {
          Ogre::CompositorManager::getSingleton().setCompositorEnabled(vp, *it, bCompositorStatus);
  ...
  bool WorkbenchState::onButtonOptionsClicked(const CEGUI::EventArgs& evt)
  {
    ...
    std::string optionsLayoutFile = levelConfig_[m_preferredSkinConfig]["optionslayout"].getAsStringOrDefaultVal ("workbenchoptions.layout");
    ...
    try
    {
      ...
      {
        workbenchOptionsWindow = ceguiSysPtr->getDefaultGUIContext ().getRootWindow ()->getChild (expectedOptionsWindowName.c_str ());
      ...
      else
      {
        ...
        CEGUI::PushButton* btn;
        btn = static_cast<CEGUI::PushButton*> (utils::CEGUIHelperUtil::getWindow ("WorkbenchOptionsRoot/WorkbenchOptions/__auto_closebutton__"));
        addWidgetMouseEffects("WorkbenchOptionsRoot/WorkbenchOptions/__auto_closebutton__");
        if (btn)
        {
          btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonOptionsCloseClicked, this));
        }

        btn = static_cast<CEGUI::PushButton*> (utils::CEGUIHelperUtil::getWindow ("WorkbenchOptionsRoot/WorkbenchOptions/ApplyAndRestart"));
        addWidgetMouseEffects("WorkbenchOptionsRoot/WorkbenchOptions/ApplyAndRestart");
        if (btn)
        {
          btn->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&WorkbenchState::onButtonOptionsApplyClicked, this));
        }

        CEGUI::RadioButton* checkbox_001 = static_cast<CEGUI::RadioButton*> (utils::CEGUIHelperUtil::getWindow ("WorkbenchOptionsRoot/WorkbenchOptions/PreferredSkinGrouper/__auto_contentpane__/SFX_RB_Radio1"));
        addWidgetMouseEffects("WorkbenchOptionsRoot/WorkbenchOptions/PreferredSkinGrouper/__auto_contentpane__/SFX_RB_Radio1");
        if (checkbox_001)
        {
        ...
        CEGUI::RadioButton* checkbox_002 = static_cast<CEGUI::RadioButton*> (utils::CEGUIHelperUtil::getWindow ("WorkbenchOptionsRoot/WorkbenchOptions/PreferredSkinGrouper/__auto_contentpane__/SFX_RB_Radio2"));
        addWidgetMouseEffects("WorkbenchOptionsRoot/WorkbenchOptions/PreferredSkinGrouper/__auto_contentpane__/SFX_RB_Radio2");
        if (checkbox_002)
        {
        ...
        CEGUI::FrameWindow* widget = static_cast<CEGUI::FrameWindow*> (utils::CEGUIHelperUtil::getWindow ("WorkbenchOptionsRoot/WorkbenchOptions"));
        if (widget)
        {
          // Toggle the roll-up event twice to invalidate the contents of the frame.
          // The regular invalidation doesn't seem to do anything. Neither is re-setting properties that would cause a redraw.
          widget->toggleRollup();
          widget->toggleRollup();
        }
      }
  ...
  bool WorkbenchState::onButtonOptionsApplyClicked (const CEGUI::EventArgs& evt)
  {
    ...
    std::string widgetName ("WorkbenchOptionsRoot/WorkbenchOptions/PreferredSkinGrouper/__auto_contentpane__/SFX_RB_Radio1");
  ...
  {
    //CEGUI::GUIContextEventArgs localArgs = static_cast<CEGUI::GUIContextEventArgs> (evt);
    ...
    if (wndEventArgs.window)
    {
      ...
      //float posX = CEGUI::CoordConverter::windowToScreenX (*wndEventArgs.window, 0);
      //float posY = CEGUI::CoordConverter::windowToScreenY (*wndEventArgs.window, CEGUI::UDim (1.0,0));
      float posX = 0;
      float posY = 0;
      posX = oldMouseX_ + 1;
      posY = oldMouseY_ + 1;
    ...
    CEGUI::Window* bar = win_mgr.createWindow (widgetType
                          , popupMenuName
                          );
  ...
  bool WorkbenchState::onButtonGetRandomValueClicked (const CEGUI::EventArgs& evt)
  {
    ...
    // Update the level label
    std::string btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopRightSelectionFrame/OptionsWindowTab/Sample_Tab_1/ButtonGetRandomProgressValue");
    if (utils::CEGUIHelperUtil::getWindow (btnName1))
    {
      GTRACE (3, "onButtonGetRandomValueClicked value is " << myval);
      // Update the level label
      btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopRightSelectionFrame/OptionsWindowTab/Sample_Tab_1/Sample_ProgressBar");
  ...
  bool WorkbenchState::onScrollValueChange (const CEGUI::EventArgs& evt)
  {
    ...
    // Update the level label
    std::string btnName1 = utils::CEGUIHelperUtil::getNameForWidget ("TopRightSelectionFrame/OptionsWindowTab/Sample_Tab_1/Sample_ProgressSlider");
    CEGUI::Scrollbar * scroller = static_cast<CEGUI::Scrollbar*> (utils::CEGUIHelperUtil::getWindow (btnName1));
    if (scroller)
    {
      ...
      bool apply_click_effect)
  {
    ...
    if (widget)
    {
      ...
      {
        widget->subscribeEvent(CEGUI::Window::EventMouseEntersArea, CEGUI::Event::Subscriber(&WorkbenchState::onWidgetHover, this));
      }
      if (apply_click_effect)
      {
        widget->subscribeEvent(CEGUI::Window::EventMouseClick, CEGUI::Event::Subscriber(&WorkbenchState::onWidgetClick, this));
      }
    }
    else
    {
  ...
  void printAllKids (Ogre::SceneNode * rootNode, int level = 0)
  {
    ...
    while (it.hasMoreElements ())
    {
      ...
      {
        Ogre::MovableObject *movable = dynamic_cast <Ogre::MovableObject*> (it2.getNext ());
        GTRACE (3, "attached: " << movable->getName () << "; " << movable->getMovableType ());
        if (movable->getMovableType () == "Entity")
        {
          Ogre::Entity * ent = dynamic_cast <Ogre::Entity*> (movable);
  ...
  void setMaterialForEntities (Ogre::SceneManager * sceneManager, std::string materialName, std::vector <std::string> entities)
  {
  ...
  bool WorkbenchState::renderScene ()
  {
      ...
      if (win_mgr)
      {
        ...
        if (label)
        {
          ...
          case 4:
            {
              ...
              if (playerCamera_->getCamera ())
              {
                ...
                //double mySplitter = playerCamera_->getCamera ()->getNearClipDistance ();
          ...
          if (! scs.isNull ())
          {
            ...
            {
              Ogre::RTShader::SubRenderState* subRenderState = enginePtr_->getShaderGenerator ()->createSubRenderState(Ogre::RTShader::IntegratedPSSM3::Type);  
            ...
            else
            {
              ...
              //enginePtr_->getShaderGenerator ()->addte
            }
#endif
          }
#if GUS_USE_RTSS
      ...
      || movingCameraLeft_ || movingCameraRight_)
    {
      ...
      if (movingCameraForward_) accel += playerCamera_->getCamera ()->getDirection();
      if (movingCameraBack_) accel -= playerCamera_->getCamera ()->getDirection();
      if (movingCameraRight_) accel += playerCamera_->getCamera ()->getRight();
      if (movingCameraLeft_) accel -= playerCamera_->getCamera ()->getRight();
    ...
    {
      GTRACE (2, " ---------------------------- Reloading group " << reloadGroupNextTick_);
      Ogre::ResourceGroupManager::LocationList dirs = Ogre::ResourceGroupManager::getSingleton ().getResourceLocationList (reloadGroupNextTick_);
      ...
      //Ogre::ResourceGroupManager::ResourceDeclarationList declList;
      //declList = Ogre::ResourceGroupManager::getSingleton ().getResourceDeclarationList (reloadGroupNextTick_);
      //for (Ogre::ResourceGroupManager::ResourceDeclarationList::iterator it = declList.begin (); it != declList.end (); ++ it)
      //{
      //  GTRACE (3, "Group has decl: " << it->resourceName << "of type " << it->resourceType);
      //}
      ...
      Ogre::StringVectorPtr ress = Ogre::ResourceGroupManager::getSingleton ().findResourceNames(reloadGroupNextTick_, "*");
      if (! ress.isNull ())
      {
      ...
      // reload the group
      Ogre::ResourceGroupManager::getSingleton ().clearResourceGroup (reloadGroupNextTick_);
      ...
      Ogre::ResourceGroupManager::getSingleton ().initialiseResourceGroup (reloadGroupNextTick_);
      Ogre::ResourceGroupManager::getSingleton ().loadResourceGroup (reloadGroupNextTick_);
      GTRACE (2, "Cleared and reloaded group: " << reloadGroupNextTick_);
      ...
      GTRACE (2, " ------------------------ Done Reloading group " << reloadGroupNextTick_);
  ...
  void WorkbenchState::applyShadowTechnique (int shadowType)
  {
    ...
    // D3D is the opposite - if you ask for PF_FLOAT16_R you get an integer format instead! You can ask for PF_FLOAT16_GR
    ...
    bool shadowCasterRenderBackFaces = levelConfig_["lighting"]["shadowCasterRenderBackFaces"].getAsBoolOrDefault (false);
    ...
    if (nDefaultSpotLightCount)
      enginePtr_->getSceneManagerPtr()->setShadowTextureCountPerLightType (Ogre::Light::LT_SPOTLIGHT, nDefaultSpotLightCount);
    if (nDefaultDirLightCount)
      enginePtr_->getSceneManagerPtr()->setShadowTextureCountPerLightType (Ogre::Light::LT_DIRECTIONAL, nDefaultDirLightCount);
    if (nDefaultOmniLightCount)
      enginePtr_->getSceneManagerPtr()->setShadowTextureCountPerLightType (Ogre::Light::LT_POINT, nDefaultOmniLightCount);

    std::vector<std::string> compositorList = levelConfig_["lighting"]["applyCompositorToShadow"].getAsStringList ();
    const unsigned numShadowRTTs = enginePtr_->getSceneManagerPtr()->getShadowTextureCount();
    GTRACE (4, "Applying " << compositorList.size () << " compositors to the " << numShadowRTTs << " shadow texture viewports");
    for (unsigned i = 0; i < numShadowRTTs; ++i)
    {
      Ogre::TexturePtr tex = enginePtr_->getSceneManagerPtr()->getShadowTexture(i);
      Ogre::Viewport *vp = tex->getBuffer()->getRenderTarget()->getViewport(0);
      ...
      for (int nIdx = indexFrom; nIdx <= indexTo; ++ nIdx)
      {
        ...
        if (sLightName.length () > 0)
        {
          ...
          if (lightType != 0)
          {
            ...
            Ogre::MaterialPtr myManualObjectMaterial = Ogre::MaterialManager::getSingleton().create(lineMaterial, staticGroupName_); 
          ...
          
          {
            ...
            Ogre::SceneNode * myNode = enginePtr_->getSceneManagerPtr ()->getRootSceneNode ()->createChildSceneNode (nodeName);
            ...
            Ogre::SceneNode * anonNode = enginePtr_->getSceneManagerPtr ()->getRootSceneNode ()->createChildSceneNode (anonName);
  ...
  void WorkbenchState::createCameras()
  {
    ...
    if (x != 0 || y != 0 || z != 0)
    {
      ...
      if (camSecPtr) camSecPtr->lookAt (Ogre::Vector3 (x, y, z));
    }
    else
    {
      ...
      if (camSecPtr) camSecPtr->setDirection (x, y, z);
    }
      ...
      for (int nIdx = indexFrom; nIdx <= indexTo; ++ nIdx)
      {
        ...
        switch (nType)
        {
            ...
            // mesh
            {
              ...
              if (meshName != "")
              {
                ...
                if (buildTangents)
                {
                  ...
                  if (! ptrMesh-> suggestTangentVectorBuildParams (Ogre::VES_TANGENT, src, dest))
                  {
                ...
                Ogre::SceneNode *snTemp = enginePtr_->getSceneManagerPtr()->getRootSceneNode()->createChildSceneNode(tempNodeName);
          ...
          default:
            {
              // plane type
              std::string planeName = sName; planeName.append ("plane");
              ...
              {
              case -3:
                planeNormalVector = Ogre::Vector3::NEGATIVE_UNIT_Z; break;
              case -2:
                planeNormalVector = Ogre::Vector3::NEGATIVE_UNIT_Y; break;
              case -1:
                planeNormalVector = Ogre::Vector3::NEGATIVE_UNIT_X; break;
              case 1:
                planeNormalVector = Ogre::Vector3::UNIT_X; break;
              case 2:
                planeNormalVector = Ogre::Vector3::UNIT_Y; break;
              case 3:
              default:
                planeNormalVector = Ogre::Vector3::UNIT_Z; break;
              }
              ...
              {
              case -3:
                planeUpVector = Ogre::Vector3::NEGATIVE_UNIT_Z; break;
              case -2:
                planeUpVector = Ogre::Vector3::NEGATIVE_UNIT_Y; break;
              case -1:
                planeUpVector = Ogre::Vector3::NEGATIVE_UNIT_X; break;
              case 1:
                planeUpVector = Ogre::Vector3::UNIT_X; break;
              case 2:
                planeUpVector = Ogre::Vector3::UNIT_Y; break;
              case 3:
              default:
                planeUpVector = Ogre::Vector3::UNIT_Z; break;
              }
              ...
              std::string planeMeshName = sName; planeMeshName.append ("mesh");
  ...
  void WorkbenchState::applyShadowTextureCasterMaterial (const std::string& materialName)
  {
    ...
    {
      GTRACE (3, "Not applying shadow texture caster material; IGNORED due to current shadow tech: " << currentShadowTech_);
    }
  }
