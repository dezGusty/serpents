...
namespace app
{
	...
	bool SerpEngine::renderCompleteScene ()
	{
		...
		try
		{
			if (! rootPtr_->_updateAllRenderTargets ())
			{
	...
	*/
	...
	{
		//static guslib::Timer guiRenderTimer;
		//guslib::TimeUnits timeSinceLastRender = guiRenderTimer.renew ();
	...
	bool SerpEngine::keyPressed (const OIS::KeyEvent &arg)
	{
		...
		// do event injection
		CEGUI::System* ceguiSysPtr = CEGUI::System::getSingletonPtr(); // TODO: use stored pointer, not singleton?
	...
	bool SerpEngine::initializeRTShaderSystem(Ogre::SceneManager* sceneMgr, const SerpEngineStartupSettings& startupSettings)
	{
